console.log("AUTH GUARD CORRECTED COMPLETE");
...
async function getUserRole(user){
  ...
  let role='user';
  try{
    const r=await fetch(`${AUTH_SUPABASE_URL}/rest/v1/user_roles?select=*&user_id=eq.${user.id}`,{headers:{apikey:AUTH_SUPABASE_KEY,Authorization:'Bearer '+await authToken()}});
    const rows=await r.json();
    if(rows && rows[0]) role=rows[0].role || 'user';
    else{
      await fetch(`${AUTH_SUPABASE_URL}/rest/v1/user_roles`,{method:'POST',headers:{apikey:AUTH_SUPABASE_KEY,Authorization:'Bearer '+await authToken(),'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify({user_id:user.id,email:user.email,role:'user'})});
    }
  }catch(e){console.warn('Role unavailable',e)}
  return role;
}
async function requireAuth(){
  ...
  lock.innerHTML=`<div class="auth-panel"><h1>Connexion</h1><p>Créez un compte avec n’importe quel email + mot de passe, ou connectez-vous.</p><input id="guardEmail" type="email" placeholder="Email"><input id="guardPassword" type="password" placeholder="Mot de passe"><button id="guardLogin">Se connecter</button><button id="guardSignup" class="secondary">Créer un compte</button><button id="guardReset" class="secondary">Mot de passe oublié</button><button id="guardHub" class="secondary">Retour au portail</button><div id="guardMsg" class="auth-error"></div></div>`;
  ...
  document.getElementById('guardSignup').onclick=async()=>{
    ...
    msg.textContent=error?error.message:'Compte créé. Si confirmation email activée, confirmez puis reconnectez-vous.';
  };
...
window.appAuth={
  ...
  signOut:async()=>{await authClient.auth.signOut(); location.href='/';}
};
